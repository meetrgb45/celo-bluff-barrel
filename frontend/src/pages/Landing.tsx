  ...
  return (
    <div style={{ width: '100%', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
      ...
      <nav style={{ width: '100%', maxWidth: 1100, padding: '1rem 1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 40 }}>
        <span style={{ fontSize: '0.75rem', color: '#8b7b5a', letterSpacing: '0.1em' }}>CELO SEPOLIA</span>
        {isConnected ? (
      ...
      <div style={{ flex: 1, width: '100%', maxWidth: 1100, padding: '0 1.5rem', zIndex: 30 }}>
        ...
        <section style={{ textAlign: 'center', padding: '4rem 0 3rem' }}>
          ...
          <p style={{ fontSize: '0.85rem', color: '#8b7b5a', maxWidth: 550, margin: '1rem auto 0', lineHeight: 1.6 }}>
            The fully on-chain card bluffing game with Russian Roulette. Built on Celo — commit your hand, reveal on challenge, survive the revolver.
          </p>
        ...
        <section style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem', marginBottom: '4rem' }}>
          ...
          <div style={{ borderRadius: '0.75rem', background: '#1a110d', padding: '1.5rem', border: '1px solid #3a2a1a', opacity: 0.7, display: 'flex', flexDirection: 'column' }}>
            <div style={{ marginBottom: '1rem' }}><span style={{ border: '1px solid #3a2a1a', color: '#5a4a3a', padding: '0.2rem 0.6rem', fontSize: '0.7rem', fontWeight: 'bold', letterSpacing: '0.1em' }}>SOON</span></div>
            <div style={{ width: '100%', aspectRatio: '4/3', background: '#251f12', borderRadius: '0.5rem', marginBottom: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '-0.5rem' }}>
                {['king1', 'devil1', 'queen1'].map((c, i) => (
                  <div key={i} className="playing-card" style={{ backgroundImage: `url(/playing_card/${c}.png)`, width: '3.5rem', transform: `rotate(${(i-1)*8}deg)`, marginLeft: i > 0 ? '-1rem' : 0, opacity: 0.8 }} />
                ))}
              </div>
            </div>
            <h3 style={{ fontSize: '1.3rem', color: '#e94560', marginBottom: '0.4rem' }}>Devil Mode</h3>
            <p style={{ fontSize: '0.75rem', color: '#5a4a3a', marginBottom: '1rem', flex: 1, lineHeight: 1.5 }}>One Devil card lurks in the deck. Play it and survive — or watch everyone else face the revolver.</p>
            <button className="btn" style={{ width: '100%', fontSize: '1rem', padding: '0.7rem', opacity: 0.4, cursor: 'not-allowed', borderColor: '#8b1a1a' }} disabled>COMING SOON</button>
          </div>
          ...
          <div style={{ borderRadius: '0.75rem', background: '#1a110d', padding: '1.5rem', border: '1px solid #3a2a1a', opacity: 0.7, display: 'flex', flexDirection: 'column' }}>
            ...
            <h3 style={{ fontSize: '1.3rem', color: '#c45cff', marginBottom: '0.4rem' }}>Chaos Mode</h3>
            <p style={{ fontSize: '0.75rem', color: '#5a4a3a', marginBottom: '1rem', flex: 1, lineHeight: 1.5 }}>Winners shoot opponents. The Chaos card makes everyone pull the trigger simultaneously.</p>
            <button className="btn" style={{ width: '100%', fontSize: '1rem', padding: '0.7rem', opacity: 0.4, cursor: 'not-allowed', borderColor: '#6b1a8b' }} disabled>COMING SOON</button>
          </div>
        </section>
        ...
        <section style={{ padding: '3rem 2rem', background: '#1a110d', borderRadius: '0.75rem', border: '1px solid rgba(184,115,51,0.2)', position: 'relative', overflow: 'hidden', marginBottom: '3rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem', marginBottom: '1rem' }}>
            <h2 style={{ fontSize: '1.5rem', color: '#c9a84c' }}>Built on Celo</h2>
          </div>
          <p style={{ color: '#8b7b5a', fontSize: '0.85rem', maxWidth: 600, lineHeight: 1.7, marginBottom: '2rem' }}>
            Cards are dealt privately by the server and committed on-chain as cryptographic hashes. Challenge a player and they must reveal their hand — the contract verifies it instantly. No trusted third party.
          </p>
          <div style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap' }}>
            {['Commit-Reveal Privacy', 'On-Chain Verification', '~1s Block Time', 'Gas in USDC'].map((label, i) => (
