...
const RULES = [
  ...
  'Each round a target card is revealed',
  'Play 1–3 cards face-down, claim they are the target card',
  'Jokers are always valid (wild)',
  'Call LIAR to challenge — accused must reveal their hand',
  'Caught lying or false accusation → spin the revolver',
...
export default function Lobby() {
  ...
  useEffect(() => {
    ...
    publicClient.readContract({
      address: GAME_ADDRESS, abi: GAME_ABI, functionName: 'getStakeAmount', args: [BigInt(joinId)],
    }).then((s: any) => setInviteStake(Number(s) > 0 ? (Number(s) / 1e6).toString() : 'Free'))
      .catch(() => setInviteStake(null));
  }, [isInvite, publicClient, joinId]);
      ...
      <div className="cards-bg">
        {[
          ...
          <div key={i} className="floating-card static" style={{
            ...
            left: c.left, top: c.top, ['--rot' as any]: c.rot,
          }} />
        ))}
      </div>
      ...
      <div className="paperboard-panel" style={{ position: 'relative', zIndex: 10, width: 440, padding: '2rem', paddingTop: '5rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', alignItems: 'center' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%', alignItems: 'center' }}>
            <span style={{ fontSize: '0.85rem', color: '#c9a84c', fontWeight: 600 }}>Basic Mode</span>
            <button onClick={() => setShowRules(true)} style={{ width: 32, height: 32, borderRadius: '50%', background: '#1a1008', border: '2px solid #5a4a3a', color: '#c9a84c', fontSize: '1rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>?</button>
          </div>
          ...
          {isInvite && (
            <div style={{ width: '100%', padding: '0.6rem', background: '#c9a84c15', border: '1px solid #c9a84c40', borderRadius: '0.3rem', textAlign: 'center' }}>
              ...
              {inviteStake && <p style={{ fontSize: '0.7rem', color: '#8b7b5a', margin: '0.2rem 0 0' }}>Stake: {inviteStake === 'Free' ? 'Free' : inviteStake + ' USDC'}</p>}
            </div>
          )}
          ...
          <p style={{ color: '#7a6a5a', fontSize: '0.65rem', marginTop: '0.3rem' }}>2–4 Players • Celo Sepolia</p>
        </div>
      </div>
      ...
      {showRules && (
        <div onClick={() => setShowRules(false)} style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div onClick={(e) => e.stopPropagation()} className="paperboard-panel" style={{ width: 380, padding: '1.5rem', maxHeight: '80vh', overflow: 'auto' }}>
            <h2 style={{ fontSize: '1.3rem', color: '#2a1a0a', marginBottom: '1rem', textAlign: 'center' }}>Basic Mode Rules</h2>
